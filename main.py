...
from ddgs import DDGS  # НОВАЯ БИБЛИОТЕКА
import requests
    ...
    try:
        ...
        with DDGS() as ddgs:
            for result in ddgs.text(query, max_results=num_results):
                # Фильтруем результаты на русском языке
        ...
        print(f"✅ Найдено русскоязычных результатов: {len(results)}")
        return results
...
def fetch_page_text(url: str, max_chars: int = 3000):
    try:
        ...
        print(f"HTTP ошибка при чтении {url}: {e}")
        return ""
    except Exception as e:
...
def ask_with_search(query: str) -> dict:
    ...
    if not results:
        return {'answer': '❌ Не удалось найти информацию по вашему запросу', 'sources': []}
    ...
    if not context_parts:
        return {'answer': '❌ Не удалось прочитать страницы с результатами', 'sources': sources}
